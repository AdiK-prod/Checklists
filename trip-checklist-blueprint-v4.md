...
**Version:** 4.1 — Final Terminology Lock: Template → Section → Category → Item  
...
This is the absolute source of truth. Never deviate. Apply to all UI copy,
code comments, variable names, component names, and prompts.

| Term | Definition | DB table |
|---|---|---|
| **Template** | A reusable packing blueprint. Flight abroad, Day trip, Weekend away. | `templates` |
| **Section** | A card in the UI. Either shared (Documents, Essentials, Misc.) or a named person (Mum, Tom, Sara). Renders as a collapsible card. | `template_sections` / `checklist_sections` |
| **Category** | A label row INSIDE a section card. Clothing, Medications, Travel docs. Visible to the user. NOT a card — a label row. | `template_subcategories` / `checklist_subcategories` |
| **Item** | A single checkable row. Passport, Summer clothes, Sunscreen. | `template_items` / `checklist_items` |

### The full hierarchy

```
Template: Flight abroad
  └── Section: Documents          (shared — renders as card)
        └── Category: Travel      (label row inside card)
              └── Item: Passport
              └── Item: Boarding pass
  └── Section: Essentials         (shared — renders as card)
        └── Category: Tech
              └── Item: Travel adapter
  └── Section: Misc.              (shared — catch-all, always last card)
        └── Category: Items       (default, auto-managed)
              └── Item: (quick-adds land here)
  └── Section: Mum                (person — renders as card)
        └── Category: Clothing
              └── Item: Summer clothes
        └── Category: Medications
              └── Item: (empty)
  └── Section: Tom                (person — renders as card)
        └── Category: Clothing
              └── Item: Summer clothes
```

### How it renders on the trip page

```
┌─ Mum ──────────────────── 3/7 ▾ ┐  ← Section header (avatar + name)
│  CLOTHING                         │  ← Category label row (11px uppercase)
│  ⠿ ☐ Summer clothes              │  ← Item
│  ⠿ ☐ Swimwear                    │  ← Item
│  MEDICATIONS                      │  ← Category label row
│  ⠿ ☐ Contraceptive pill          │  ← Item
│  [Add item...] [Add]              │  ← Always at bottom
└───────────────────────────────────┘

┌─ Documents ────────────── 0/3 ▾ ┐  ← Section header (icon + name, shared)
│  TRAVEL                           │  ← Category label row
│  ⠿ ☐ Passport                    │  ← Item
│  ⠿ ☐ Boarding pass               │  ← Item
│  [Add item...] [Add]              │
└───────────────────────────────────┘
...
- **Section** = the card. Created by "Add section" (shared or person).
- **Category** = a label row inside the card. Created by "Add category" inside a section.
- **Item** = a checkable row inside a category.
- The word "subcategory" NEVER appears in any UI copy. It is a DB implementation
  detail only (table is named template_subcategories for historical reasons).
- "Misc." is a shared section (card), always sorted last. Its default category
  is "Items". Auto-created when a user adds an item with no section selected.
- On the templates screen: sections render as a flat list (editing surface).
- On the trip page: sections render as distinct cards with visual treatment
  (shared sections get category icons, person sections get avatar circles).
...
-- Template sections: either a shared section (Documents, Essentials, Misc.)
-- or a named person section (Mum, Tom). Each renders as a card in the UI.
-- section_type = 'shared'  → shared section. No member_id.
-- section_type = 'person'  → person section. member_id references household_members.
...
-- Template categories: label rows inside a section card. e.g. Clothing, Medications, Travel.
-- Renders as a plain uppercase label row inside a section card — NOT a separate card.
-- "Add category" in the UI creates these. Every section needs at least one category
-- for items to attach to. Default category name is "Items" when auto-created.
...
-- Checklist sections: live copy of template sections for a specific trip.
-- Shared sections always copied. Person sections only copied for travelling members.
...
-- Checklist categories: label rows inside a section card on a live trip.
-- Copied from template on trip creation. Auto-created ("Items") when a new section is added.
-- Users create these via "Add category" inside a section on the trip page.
...
## Implementation State (as of v4.0)

The following has been built and is live at checklists-nine.vercel.app.
Modules marked ✅ are complete. Modules marked 🔄 need alignment work.

### What is built and working
- ✅ Auth (sign up, sign in, sign out, invite link)
- ✅ Onboarding (household name, family members, template seeding)
- ✅ Trips dashboard (upcoming + archive)
- ✅ New trip wizard (4 steps including AI suggestions)
- ✅ Trip page (per-category cards, shared + person tracks)
- ✅ Pack templates screen (view, add category, add items)
- ✅ Settings (household management, invite, sign out)
- ✅ Misc. as a shared category (not subcategory) — catch-all for quick adds
- ✅ Add category (collapsible panel, shared and person types)
- ✅ Rename / Remove category on templates and trip checklists
- ✅ Default "Items" subcategory auto-created on new category
- ✅ Subcategory add removed from UI — users only manage categories and items

### Known issues / next alignment work
- 🔄 Terminology in UI copy is inconsistent — must align to Template → Section → Category → Item
- 🔄 Pack templates screen shows "Subcategory (optional)" — must change to "Category (optional)"
- 🔄 "Misc. category — default" → "Misc. — default"
- 🔄 "New item label" → "New item"
- 🔄 Design system not fully applied — colours and typography need a pass
- 🔄 Template seeding uses old flat structure in some paths — needs verification
- 🔄 Keepalive cron not yet implemented (20s cold start on refresh)
